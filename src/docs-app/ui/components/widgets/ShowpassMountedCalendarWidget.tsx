...
const ShowpassMountedCalendarWidget = ({
  ...
  React.useEffect(() => {
    ...
    const initWidget = () => {
      ...
      if (window.showpass && window.showpass.tickets) {
        ...
        console.log(
          `Mounting calendar widget into container: ${containerId} using legacy method`
        );
        try {
          window.__shwps(
          ...
          console.error(
            "Error mounting Showpass calendar widget with legacy method:",
            error
          );
        }
      } else {
  ...
  return (
    <div
      id={containerId}
      className={`showpass-calendar-container min-h-[500px] ${className}`}
    >
