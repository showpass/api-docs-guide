...
const ShowpassMountedMembershipWidget = ({
  ...
  const containerId = React.useRef(`membership-widget-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`);
  ...
  React.useEffect(() => {
    ...
    const initWidget = () => {
      if (window.showpass?.tickets) {
        window.showpass.tickets.membershipPurchaseWidget(id, memoizedOptions, containerId.current);
        isInitialized.current = true;
      } else if (window.__shwps) {
        window.__shwps('tickets.membershipPurchaseWidget', id, memoizedOptions, containerId.current);
        isInitialized.current = true;
      }
    };
